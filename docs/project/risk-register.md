...
| R-004 | HIGH | OPEN | GeoIP timeout, limit lub awaria | provider port, HTTPS, 500 ms connect, 1 s response, brak retry, 503 | accepted EMP-006 + provider tests |
| R-005 | HIGH | OPEN | spoofing `Forwarded` / `X-Forwarded-For` | direct default, CIDR trust, right-to-left chain, malformed header fail-closed | accepted EMP-006 + resolver tests |
...
| R-011 | HIGH | OPEN | utrwalanie lub logowanie IP zwiększa ryzyko prywatności | memory-only raw IP, brak hash/storage, redaction i provider data minimization | EMP-006 privacy tests + schema review |
...
| R-015 | HIGH | OPEN | lokalny GeoIP stub zostaje przypadkowo aktywowany w produkcji | bean/profile guard + startup failure poza `local`/`test` + configuration test | EMP-006/010 |
...
| R-018 | HIGH | OPEN | parser IP wykonuje DNS lub akceptuje hostname | strict literal parser, brak `getByName` na niezweryfikowanym tekście, testy hostname/zone ID | EMP-006 |
| R-019 | HIGH | OPEN | prywatny lub specjalny adres jest wysyłany do publicznego GeoIP | IANA special-purpose policy przed adapterem HTTP | EMP-006 |
| R-020 | MEDIUM | OPEN | długi łańcuch proxy powoduje koszt CPU lub niejednoznaczność | 4096 znaków, maks. 20 hopów, fail-closed | EMP-006 |
| R-021 | HIGH | OPEN | wielokrotne field-lines lub redirect/body dostawcy omijają granicę zaufania | fail-closed, redirect disabled, 16 KiB bounded read, testy WireMock | EMP-006 |
