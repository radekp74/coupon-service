...
.PHONY: help docs-check bootstrap-check emp003-check emp004-refinement-check emp004-check emp006-refinement-check emp006-check emp007-check emp008-refinement-check emp008-check emp008-report-check emp009-refinement-check emp009-check emp010-refinement-check emp010-check emp011-refinement-check emp011-check delivery-check java-check docker-check compose-config docker-build docker-up docker-down docker-logs docker-smoke maven-verify verify checksums package export-source clean
...
help:
	@printf '%s\n' \
		...
		'make emp011-check   - validate the EMP-011 final repository review contract' \
...
emp011-check:
	python3 scripts/check_emp011.py
